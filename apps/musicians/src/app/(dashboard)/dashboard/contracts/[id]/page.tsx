...
const STATUS_CONFIG: Record<string, { label: string; color: string; nextStatus?: string; nextLabel?: string }> = {
  DRAFT: { label: 'Koncept', color: 'bg-gray-100 text-gray-700', nextStatus: 'SENT', nextLabel: 'Odeslat klientovi' },
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {contract.clientInfo.company && (
                  <p className="text-sm text-gray-500">{contract.clientInfo.company}</p>
                )}
              </div>
            </div>
          </CardContent>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-500">Datum akce</p>
                <p className="font-medium">
                  {new Date(contract.eventDetails.date).toLocaleDateString('cs-CZ')}
                </p>
                {contract.eventDetails.time && (
                  <p className="text-sm text-gray-500">{contract.eventDetails.time}</p>
                )}
              </div>
            </div>
          </CardContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {contract.clauses
                  .sort((a, b) => a.order - b.order)
                  .map((clause) => (
                    <div key={clause.clauseId} className="p-4 bg-gray-50 rounded-lg">
                      <h4 className="font-medium text-gray-900 mb-2">{clause.title}</h4>
                      <p className="text-gray-700 text-sm">{clause.content}</p>
                    </div>
                  ))}
              </CardContent>
            </Card>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              {contract.performerInfo.address && <p className="text-gray-600">{contract.performerInfo.address}</p>}
              {contract.performerInfo.ico && <p className="text-gray-600">IČO: {contract.performerInfo.ico}</p>}
              {contract.performerInfo.dic && <p className="text-gray-600">DIČ: {contract.performerInfo.dic}</p>}
              {contract.performerInfo.email && <p className="text-gray-600">{contract.performerInfo.email}</p>}
              {contract.performerInfo.phone && <p className="text-gray-600">{contract.performerInfo.phone}</p>}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              <p className="font-medium">{contract.clientInfo.name}</p>
              {contract.clientInfo.company && <p className="text-gray-600">{contract.clientInfo.company}</p>}
              {contract.clientInfo.address && <p className="text-gray-600">{contract.clientInfo.address}</p>}
              {contract.clientInfo.email && <p className="text-gray-600">{contract.clientInfo.email}</p>}
              {contract.clientInfo.phone && <p className="text-gray-600">{contract.clientInfo.phone}</p>}
            </CardContent>
          </Card>
          ...
          {contract.gig && (
            <Card>
              ...
              <CardContent>
                <Link href={`/dashboard/gigs/${contract.gig.id}`} className="flex items-center gap-2 text-primary-600 hover:underline">
              ...
              <CardContent className="space-y-2">
                {contract.versions.map((v) => (
                  <Link
                    ...
                    className={`block p-2 rounded hover:bg-gray-50 ${v.id === contract.id ? 'bg-primary-50' : ''}`}
                  >
