...
export default function GenerateSetlistPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      router.push(`/dashboard/setlists/${savedSetlist.id}`)
    } catch (err) {
            ...
            <CardContent>
              {isLoadingRepertoire ? (
                ...
                <div className="text-center py-4">
                  ...
                  <p className="text-gray-600">Nemáte žádné písně v repertoáru</p>
                  <Button variant="outline" className="mt-3" asChild>
                    <Link href="/dashboard/repertoire/new">Přidat písně</Link>
                  </Button>
                </div>
              ) : (
                <div className="space-y-3">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">Počet písní:</span>
                    <span className="font-medium">{repertoire.length}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">Celková délka:</span>
                      ...
                      {set.songs.map((song, idx) => (
                        ...
                        >
                          ...
                          <div className="text-right">
                            <Badge className={`${moodColors[song.mood] || 'bg-gray-100 text-gray-700'} text-xs`}>
