...
export default function EditShotListPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/dashboard/shot-lists/${params.id}`)
    } catch (error) {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      {/* Header */}
      <div>
        <Link href={`/dashboard/shot-lists/${params.id}`} className="inline-flex items-center text-gray-600 hover:text-gray-900 mb-4">
          <ArrowLeft className="w-4 h-4 mr-2" />
          Back to Shot List
        </Link>
        <h1 className="text-3xl font-bold text-gray-900">Edit Shot List</h1>
        <p className="text-gray-600 mt-1">Update shot list details</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Lighting Plan</label>
              <textarea
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href={`/dashboard/shot-lists/${params.id}`}>
