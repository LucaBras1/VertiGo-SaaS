  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <Link href="/dashboard/shoots" className="inline-flex items-center text-gray-600 hover:text-gray-900 mb-2">
          ...
          <p className="text-gray-600 mt-1">{shoot.package.client.name}</p>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/dashboard/shoots/${params.id}/edit`}>
      ...
      <Card className="p-4">
        <div className="flex flex-wrap gap-2">
          {!shoot.shotList && (
            <Link href={`/dashboard/shot-lists/new?shootId=${shoot.id}&packageId=${shoot.package.id}`}>
              <Button size="sm">
          ...
          <Link href={`/dashboard/galleries/new?shootId=${shoot.id}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <div className="space-y-3">
              ...
              {shoot.lightingNotes && (
                <div>
                  <p className="text-sm text-gray-600">Lighting Notes</p>
                  <p className="text-gray-700">{shoot.lightingNotes}</p>
                </div>
              )}
            </div>
          </Card>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                {!shoot.shotList && (
                  <Link href={`/dashboard/shot-lists/new?shootId=${shoot.id}`}>
            ...
            {shoot.shotList ? (
              <Link href={`/dashboard/shot-lists/${shoot.shotList.id}`}
                className="block p-3 rounded-lg border hover:bg-gray-50 transition-colors">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium">{shoot.shotList.name}</p>
                    <p className="text-sm text-gray-500">{shoot.shotList.totalShots} shots</p>
                  </div>
              ...
              <p className="text-gray-400">No shot list created yet</p>
            )}
          </Card>
                ...
                {shoot.galleries.map(gallery => (
                  <Link key={gallery.id} href={`/dashboard/galleries/${gallery.id}`}
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{gallery.totalPhotos} photos</p>
                    </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <Link href={`/dashboard/packages/${shoot.package.id}`}
              className="block p-3 rounded-lg border hover:bg-gray-50 transition-colors">
              <p className="font-medium">{shoot.package.title}</p>
              <p className="text-sm text-gray-500">{shoot.package.eventType}</p>
            </Link>
          </Card>
          ...
          <Card>
            ...
            <div className="space-y-2">
              <Link href={`/dashboard/clients/${shoot.package.client.id}`}
                className="font-medium text-amber-600 hover:text-amber-700">
                {shoot.package.client.name}
              </Link>
              <p className="text-sm text-gray-600">{shoot.package.client.email}</p>
              {shoot.package.client.phone && (
                <p className="text-sm text-gray-600">{shoot.package.client.phone}</p>
              )}
            </div>
          </Card>
        </div>
