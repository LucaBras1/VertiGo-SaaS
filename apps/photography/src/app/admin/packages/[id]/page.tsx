  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <Link href="/admin/packages" className="inline-flex items-center text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:text-neutral-100 mb-2">
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back to Packages
          </Link>
          <h1 className="text-3xl font-bold text-neutral-900 dark:text-neutral-100">{pkg.title}</h1>
          <p className="text-neutral-600 dark:text-neutral-400 mt-1">{pkg.client.name} &bull; {pkg.eventType}</p>
        </div>
      ...
      <Card className="p-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <Link href={`/admin/shoots/new?packageId=${pkg.id}`}>
              <Button size="sm" variant="ghost">
                <Calendar className="w-4 h-4 mr-1" />
                Add Shoot
              </Button>
            </Link>
            <Link href={`/admin/shot-lists/new?packageId=${pkg.id}`}>
              <Button size="sm" variant="ghost">
                <ListChecks className="w-4 h-4 mr-1" />
                Create Shot List
              </Button>
            </Link>
            <Link href={`/admin/invoices/new?packageId=${pkg.id}`}>
              <Button size="sm" variant="ghost">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-neutral-600 dark:text-neutral-400">Event Type</p>
              ...
              <div>
                <p className="text-sm text-neutral-600 dark:text-neutral-400">Shot Count</p>
                <p className="font-medium">{pkg.shotCount || '-'} photos</p>
              </div>
              <div>
                <p className="text-sm text-neutral-600 dark:text-neutral-400">Delivery Days</p>
                <p className="font-medium">{pkg.deliveryDays || '-'} days</p>
              </div>
              <div>
                <p className="text-sm text-neutral-600 dark:text-neutral-400">Editing Hours</p>
                <p className="font-medium">{pkg.editingHours || '-'} hours</p>
              </div>
              <div>
                <p className="text-sm text-neutral-600 dark:text-neutral-400">Second Shooter</p>
          ...
          <Card>
            ...
            <div className="space-y-4">
              <div>
                <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-2">Style Tags</p>
                <div className="flex flex-wrap gap-2">
                  {pkg.styleTags && pkg.styleTags.length > 0 ? pkg.styleTags.map((tag, idx) => (
                    <Badge key={idx} variant="default">{tag}</Badge>
                  )) : <span className="text-neutral-400 dark:text-neutral-500">No tags</span>}
                </div>
              </div>
              <div>
                <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-2">Equipment</p>
                <div className="flex flex-wrap gap-2">
                  {pkg.equipment && pkg.equipment.length > 0 ? pkg.equipment.map((item, idx) => (
                    <Badge key={idx} variant="info">{item}</Badge>
                  )) : <span className="text-neutral-400 dark:text-neutral-500">Not specified</span>}
                </div>
              </div>
              <div>
                <p className="text-sm text-neutral-600 dark:text-neutral-400">RAW Files Included</p>
          ...
          {pkg.notes && (
            <Card>
              ...
              <p className="text-neutral-700 dark:text-neutral-300 whitespace-pre-wrap">{pkg.notes}</p>
            </Card>
          )}
        </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <div className="space-y-2">
              <Link href={`/admin/clients/${pkg.client.id}`} className="font-medium text-amber-600 hover:text-amber-700">
                {pkg.client.name}
              </Link>
              <p className="text-sm text-neutral-600 dark:text-neutral-400">{pkg.client.email}</p>
              {pkg.client.phone && <p className="text-sm text-neutral-600 dark:text-neutral-400">{pkg.client.phone}</p>}
            </div>
          </Card>
          ...
          <Card>
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between">
                <span className="text-neutral-600 dark:text-neutral-400">Travel</span>
          ...
          <Card>
            ...
            {!pkg.shoots || pkg.shoots.length === 0 ? (
              <p className="text-neutral-400 dark:text-neutral-500 text-sm">No shoots scheduled</p>
            ) : (
              <div className="space-y-2">
                {pkg.shoots.map(shoot => (
                  <Link key={shoot.id} href={`/admin/shoots/${shoot.id}`}
                    className="block p-2 rounded hover:bg-neutral-50 dark:bg-neutral-800">
                    <p className="font-medium text-sm">{new Date(shoot.date).toLocaleDateString()}</p>
                    <p className="text-xs text-neutral-500 dark:text-neutral-400">{shoot.venueName || 'No venue'}</p>
                  </Link>
                ))}
              </div>
            )}
          ...
          <Card>
            ...
            {!pkg.shotLists || pkg.shotLists.length === 0 ? (
              <p className="text-neutral-400 dark:text-neutral-500 text-sm">No shot lists</p>
            ) : (
              <div className="space-y-2">
          ...
          <Card>
            ...
            {!pkg.invoices || pkg.invoices.length === 0 ? (
              <p className="text-neutral-400 dark:text-neutral-500 text-sm">No invoices</p>
            ) : (
              <div className="space-y-2">
