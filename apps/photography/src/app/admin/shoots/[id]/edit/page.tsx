...
export default function EditShootPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/admin/shoots/${params.id}`)
    } catch (error) {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      {/* Header */}
      <div>
        <Link href={`/admin/shoots/${params.id}`} className="inline-flex items-center text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:text-neutral-100 mb-4">
          <ArrowLeft className="w-4 h-4 mr-2" />
          Back to Shoot
        </Link>
        <h1 className="text-3xl font-bold text-neutral-900 dark:text-neutral-100">Edit Shoot</h1>
        <p className="text-neutral-600 dark:text-neutral-400 mt-1">Update shoot details</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href={`/admin/shoots/${params.id}`}>
