...
export function CommandPalette({ commands, groups }: CommandPaletteProps) {
  ...
  const allGroups = groups || [...new Set(commands.map((cmd) => cmd.group))]
  const groupedResults = allGroups
    .map((group) => ({
      group,
